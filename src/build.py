...
class mortier(entite):
    ...
    def __init__(self,xx,yy,cooldown,degats,rangge,vie):
        ...
        self.range=rangge
        self.range=600
        #self.ligneDeMire = [self.range,self.range*2+self.hitbox.height,self.range-500,(self.range-500)*2+self.hitbox.height]
        self.ligneDeMireG=pygame.Rect((self.hitbox.x-self.range, self.hitbox.y-self.range, self.range*2+self.hitbox.height, self.range*2+self.hitbox.height))
        self.ligneDeMireP=pygame.Rect((self.hitbox.x-self.range-500,self.hitbox.y-(self.range-500),(self.range-500)*2+self.hitbox.height,(self.range-500)*2+self.hitbox.height))
        self.cooldown=100
        self.degats=1
        self.vie=10
        self.projectiles = []
                ...
                for entite in entities:
                    if entite.hitbox.x>(self.hitbox.x-self.range) and  entite.hitbox.x<(self.hitbox.x+self.range) and entite.hitbox.y>(self.hitbox.y-self.range) and  entite.hitbox.y<(self.hitbox.y+self.range):
                        if not (entite.hitbox.x>(self.hitbox.x-self.range*0.5) and  entite.hitbox.x<(self.hitbox.x+self.range*0.5) and entite.hitbox.y>(self.hitbox.y-self.range*0.5) and  entite.hitbox.y<(self.hitbox.y+self.range*0.5)):
                            shoot = ProjectileMortier(self.hitbox.x,self.hitbox.y,5,3,entite.hitbox.x,entite.hitbox.y)
